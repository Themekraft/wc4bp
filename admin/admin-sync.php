...
function wc4bp_screen_sync() { ?>
	...
	<div style="overflow: auto;">
		...
		<span style="font-size: 13px; float:right;">Proudly brought to you by <a href="http://themekraft.com/"
		                                                                         target="_new">Themekraft</a>.</span>
	</div>
	<br>
	<?php
...
function wc4bp_shop_profile_sync() { ?>
	...
	<p>The billing and shipping address fields are WooCommerce default user fields. You can sync all default WooCommerce
		customer fields with BuddyPress.</p>
	<p>During checkout or if a user edit his profile all fields will be synced automaticly.</p>
	<p>If you have already customers and start using WooCommerce BuddyPress Integration on an existing store, you need
		to sync your user once.</p>
	<p>The Customer Billing and Shipping Address fields will be created in BuddyPress during the plugin installation,
		but the user sync can take a while depends on the size of your user base and needs to be done once after the
		first install.</p>

	<br>
	<a onclick="document.getElementById('wc_default_fields').style.display='';return false;" href=""
	...
	<input type="button" id="wc4bp_sync_wc_user_with_bp_ajax" name="wc4bp_options_sync[wc_bp_sync]"
	...
	<table class="wp-list-table widefat fixed users">
		...
		<?php foreach ( $query as $q ) {
			...
			<tr id="user-1" class="alternate">
				...
				<td class="email column-email"><a href="<?php echo get_the_author_meta( 'user_email', $q->ID ); ?>"
				                                  title="E-mail: <?php echo get_the_author_meta( 'user_email', $q->ID ); ?>"><?php echo get_the_author_meta( 'user_email', $q->ID ); ?></a>
				</td>
...
function wc4bp_change_xprofile_visabilyty_default() { ?>
	...
	<input type="submit" class="button" name="wc4bp_options_sync[change_xprofile_visabilyty_field_default]"
	       value="Change now">
	<?php
...
function wc4bp_change_xprofile_allow_custom_visibility() {
	...
	<select name="wc4bp_options_sync[custom_visibility]">
		<option value="allowed" <?php echo selected( $wc4bp_options_sync['custom_visibility'], 'allowed', false ); ?>>
			Let members change this field's visibility
		</option>
