 ...
 *
 */
define( 'TK_WC4BP__EDD_DOWNLOAD_ID', '15105' ); // you should use your own CONSTANT name, and be sure to replace it throughout this file
 ...
 * @return bool
 */
function do_wc4bp_edd2fs_license_migration(
	$edd_download_id,
	$edd_license_key,
	$edd_store_url,
	$redirect = false
 ...
 */
function spawn_wc4bp_edd2fs_license_migration( $edd_download_id ) {
	global $wp;
 ...
 * @return string|bool
 */
function wc4bp_non_blocking_edd2fs_license_migration(
	$edd_download_id,
	$edd_license_key,
	$edd_store_url,
	$is_blocking = false
	...
	if ( ! $is_blocking && ! $in_migration ) {
		...
		return spawn_wc4bp_edd2fs_license_migration( $edd_download_id );
	} else {
		if ( $is_blocking ||
			...
			$success = do_wc4bp_edd2fs_license_migration(
				$edd_download_id,
				$edd_license_key,
				$edd_store_url
			);
 ...
 * @return bool
 */
function wc4bp_edd_activate_license( $license_key ) {
	// Call the custom API.
	$response = wp_remote_post(
		TK__EDD_STORE_URL,
		array(
			...
			'body'      => array(
				...
				'license'    => $license_key,
				'item_id'    => TK_WC4BP__EDD_DOWNLOAD_ID,
				'url'        => home_url()
			)
		)
	);
	...
	if ( 'valid' === $license_data->license ) {
		...
		update_option( 'edd_wc4bp_license_key', $license_key );
	} else {
		return false;
	}
 ...
 */
function wc4bp_try_migrate_on_activation( $response, $args ) {
	...
	if ( ( is_object( $response->error ) && 'invalid_license_key' === $response->error->code ) ||
		...
		if ( wc4bp_edd_activate_license( $license_key ) ) {
			// Successfully activated license on EDD, try to migrate to Freemius.
			if ( do_wc4bp_edd2fs_license_migration(
				TK_WC4BP__EDD_DOWNLOAD_ID,
				$license_key,
				TK__EDD_STORE_URL,
				true
			) ) {
...
if ( ! defined( 'DOING_CRON' ) ) {
	 ...
	 */
	WC4BP_Loader::getFreemius()->add_filter( 'after_install_failure', 'wc4bp_try_migrate_on_activation', 10, 2 );

	if ( ! empty( $license_key ) ) {
		if ( ! defined( 'DOING_AJAX' ) ) {
			wc4bp_non_blocking_edd2fs_license_migration(
				TK_WC4BP__EDD_DOWNLOAD_ID,
				$license_key,
				TK__EDD_STORE_URL
			);
